...
def make_figure_7a(ax):
		...
		#open CSV file
		with open('../fig7a.csv') as csvDataFile:
		...
		def autolabel(rects, xpos='center',jr=0):
			...
			for rect in rects:
				...
				ax.annotate(
					...
					(x_value,12-4*(jr//3)),		# Place label at end of the bar
					xytext=(0, space),			# Vertically shift label by `space`
					textcoords="offset points", # Interpret `xytext` as offset in points
					ha='center',				# Horizontally center label
					va=va,fontsize=20)			# Vertically align label differently for
												# positive and negative values.
				if(y_value>0): ax.plot([x_value,x_value],[12-4*(jr//3)+0.5,y_value+0.5],linestyle='--',color='black',linewidth=0.8)
				else:		   ax.plot([x_value,x_value],[12-4*(jr//3)+0.5,0.5],linestyle='--',color='black',linewidth=0.8)
				jr += 1
			return jr
