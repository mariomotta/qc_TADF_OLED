...
def make_figure_5b(ax):
    ...
    #open CSV file
    with open('../fig5b_t1.csv') as csvDataFile:
    ...
    def autolabel(rects, xpos='center',jr=0):
        ...
        for rect in rects:
            ...
            if(y_value>0): ax.plot([x_value,x_value],[17-4*(jr//3)+0.5,y_value+0.5],linestyle='--',color='black',linewidth=0.8)
            else:          ax.plot([x_value,x_value],[17-4*(jr//3)+0.5,0.5],linestyle='--',color='black',linewidth=0.8)
            jr += 1
        return jr
    ...
    ax.set_title('(b) $T_1$ states', fontsize=24,pad=10)
    ax.set_ylabel(r' $ \Delta \Delta E_{ST}$ [mHa]', fontsize=18)
